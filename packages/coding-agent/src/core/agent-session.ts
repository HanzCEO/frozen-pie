...
function _withoutDeletedHeaders(headers: ProviderHeaders | undefined): Record<string, string> | undefined {
	return headers
...
function _estimateMessagesTokens(messages: AgentMessage[]): number {
	let chars = 0;
		...
		if ("content" in message) {
			if (typeof message.content === "string") chars += message.content.length;
				...
				for (const c of message.content) {
					if ("text" in c && typeof c.text === "string") chars += c.text.length;
				}
			}
		}
	}
...
export class AgentSession {
	...
	getContextUsage(): ContextUsage | undefined {
		...
		for (const msg of this.messages) {
			if (msg.role === "assistant" && msg.usage) {
				tokens = msg.usage.totalTokens || msg.usage.input + msg.usage.output;
			}
		}
	...
	async exportToHtml(outputPath?: string, options: { themeName?: string } = {}): Promise<string> {
		...
		const toolRenderer: any = /*createToolHtmlRenderer*/ {
